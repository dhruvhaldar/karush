...
def solve_sdp_barrier(C, A_list, b, X0, initial_mu=1.0, tol=1e-6, max_iter=20):
    ...
    if isinstance(initial_mu, bool) or not isinstance(initial_mu, (int, float, np.number)) or np.isnan(initial_mu) or initial_mu <= 0:
    ...
    # Precompute svec(X) outside the loop to avoid O(n^2) tracking every inner iteration.
    # We update svec_X linearly when X is updated: svec_X += alpha * dx_vec
    svec_X = svec(X)
