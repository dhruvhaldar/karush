...
## 2024-08-21 - Maintain State Variables Correctly in Iterative Loops
**Learning:** When using stateful heuristics across iterations, such as Nocedal & Wright's initial line-search step size (`alpha = alpha * d_old / d_new`), modifying the state variable (`alpha`) internally within the loop (e.g., backtracking `alpha *= rho`) corrupts the state for the next iteration. This leads to severe performance regressions, such as the initial guess crashing towards zero and forcing excessive inner loop iterations.
**Action:** When a heuristic depends on the initial state of the previous iteration, explicitly introduce a separate variable (e.g., `alpha_init = alpha`) before the inner modifications begin, and use this preserved state for calculations in subsequent iterations.
