...
## 2024-05-25 - Avoid 2D Broadcasting Advanced Indexing for Submatrices
**Learning:** In NumPy, combining 2D broadcasting advanced indexing (e.g., `X_inv[idx_a[:, None], idx_a[None, :]]`) to extract submatrices is memory-intensive and slow.
**Action:** Use sequential 1D extractions (e.g., first extracting rows `X_inv_a = X_inv[idx_a]`, then extracting columns `X_inv_a[:, idx_a]`) to reduce memory footprint and improve performance.
