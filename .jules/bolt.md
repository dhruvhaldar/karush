...
## 2024-08-10 - Avoid intermediate array allocations in NumPy via out parameters
**Learning:** In NumPy, combining mathematical operations on a single line (like `rhs[dim_vec:] = b - A_mat @ svec_X`) can avoid explicitly allocating an intermediate `residuals` array if you don't need it. Additionally, utilizing the `out=` parameter for ufuncs (e.g., `np.negative(grad, out=rhs[:dim])`) or `np.add(A, B, out=C)` avoids creating temporary arrays before assigning them to a target, providing memory allocation savings inside tight loops.
**Action:** Use `out=` parameters for NumPy ufuncs and explicitly pre-allocate arrays outside of iterative loops (e.g., `X_new = np.empty_like(X)`) to write results in-place without triggering repeated memory allocations.
