...
## 2024-05-16 - Dynamic state vector tracking in SDP barrier solver
**Learning:** In iterative solvers tracking a dynamic matrix state `X`, applying linear operators like `svec(X)` inside the inner loop is an unnecessary performance overhead.
**Action:** Cache the operator's output (`svec_X = svec(X)`) outside the loop and apply linear updates directly to the cached vector (`svec_X += alpha * dx_vec` corresponding to `X += alpha * dX`) to eliminate redundant O(n^2) tracking operations.
