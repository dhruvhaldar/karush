...
## 2024-05-18 - Boolean Bypass of Type and Value Validation
**Vulnerability:** A type bypass vulnerability existed where boolean values (`True`/`False`) were accepted as valid parameters for `tol`, `max_iter`, `mu0`, and `initial_mu`, bypassing integer and float checks because booleans are a subclass of integers in Python. When `tol=True` is provided, it evaluates as `1`, leading to very early and incorrect termination. When `tol=False` is provided, the `tol <= 0` condition throws an error (since `0 <= 0`), but `max_iter=True` evaluates as `1` max iteration.
**Learning:** Python's `isinstance(val, (int, float))` naturally returns `True` for boolean values because `bool` inherits from `int`. Relying solely on these type checks allows logic errors and potentially bypasses safety limits if `True` / `False` are evaluated numerically.
**Prevention:** Explicitly reject boolean types in parameter validation using `isinstance(val, bool)` in conjunction with integer or float checks, e.g., `not isinstance(val, (int, float)) or isinstance(val, bool)`.
