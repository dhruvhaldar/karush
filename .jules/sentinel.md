...
## 2024-05-18 - [Dimension Validation for Gradients]
**Vulnerability:** User-provided functions (`grad_f`) might return incorrectly dimensioned arrays (e.g., 2D matrix instead of 1D vector). Passing these arrays to `np.linalg.solve` or evaluating boolean conditions on them causes unhandled `ValueError` or `LinAlgError` exceptions (e.g., `ValueError: The truth value of an array with more than one element is ambiguous`).
**Learning:** Checking for non-finite values (`NaN`/`Inf`) is insufficient for user-provided array outputs. Array dimensions must also be validated to prevent DoS via unhandled exceptions.
**Prevention:** Explicitly validate array dimensions (`if g.ndim != 1: raise ValueError(...)`) for inputs and user-provided function return values before using them in matrix operations or line search conditions.
