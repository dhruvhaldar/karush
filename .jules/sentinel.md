...
## 2025-02-23 - DoS Prevention from Unhandled Python Lists in Objective Functions
**Vulnerability:** Unconstrained optimizers (Newton, BFGS, Conjugate Gradient) crashed (`TypeError` or `ValueError`) when the user-provided objective function `f(x)` returned standard Python lists or multi-element lists instead of scalar NumPy types.
**Learning:** Functions that interact mathematically with numpy arrays and floats must sanitize external function return values by explicitly converting them using `np.asarray(..., dtype=float)` and correctly handling truth values of multi-element arrays (`np.all()` or `np.any()`). The vulnerability arose because mathematical operators on mixed lists and floats fail unexpectedly (e.g., list `+` float raises `TypeError`).
**Prevention:** In local mathematical libraries, always cast array-like function return values to NumPy arrays via `np.asarray` before performing arithmetic operations or inequalities, and use `np.all` or `np.any` for array comparisons.
